...
import type { UpdateCashierPayload, CashierRecord } from "@/lib/types/cashiers";
...
export default function CashierManagement() {
  ...
  const [editingCashier, setEditingCashier] = useState<CashierRecord | null>(
    null,
  );
  ...
  const [suspendTarget, setSuspendTarget] = useState<CashierRecord | null>(
    null,
  );
  ...
  const { trigger: updateCashier, isMutating: isUpdating } = useUpdateCashier(
    editingCashier?.id,
    {
      onSuccess: () => {
        mutate();
  ...
  const { trigger: deleteCashier, isMutating: isDeleting } = useDeleteCashier(
    deleteTarget?.id,
    {
      onSuccess: () => {
        mutate();
  ...
  const { trigger: regeneratePin, isMutating: isRegenerating } =
    useRegenerateCashierPin(regenTarget?.id, {
      onSuccess: (result) => {
        mutate();
        setRegenTarget(null);
        if (result) {
  ...
  const { trigger: suspendCashier, isMutating: isSuspending } =
    useUpdateCashier(suspendTarget?.id, {
      onSuccess: (result) => {
        mutate();
        setSuspendTarget(null);
        if (result) {
          toast.warning(
            `${result.data.firstName}'s POS access suspended. Active sessions terminated.`,
          );
        }
      },
    });

  const { trigger: reactivateCashier, isMutating: isReactivating } =
    useUpdateCashier(reactivateTarget?.id, {
      onSuccess: (result) => {
        mutate();
  ...
  const toggleStatus = (cashier: CashierRecord) => {
    const next: CashierStatus =
      cashier.status === "active" ? "suspended" : "active";
    if (next === "suspended") {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        {cashier.status === "active" ? (
          <DropdownMenuItem
            onClick={() => setSuspendTarget(cashier)}
                ...
                {isLoading
                  ...
                  : paginatedItems.map((cashier) => (
                      <TableRow
                        key={cashier.id}
                        className={
                          cashier.status === "suspended" ? "opacity-70" : ""
                        }
                      >
                        <TableCell>
                          <div className="flex items-center gap-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {isLoading
            ...
            : paginatedItems.map((cashier) => (
                <Card
                  key={cashier.id}
                  className={`p-4 hover:shadow-md transition-shadow ${cashier.status === "suspended" ? "opacity-70" : ""}`}
                >
          ...
          {pinReveal && (
            <div className="space-y-4">
              <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
                <div className="flex justify-between items-start gap-3 text-sm">
                  ...
                  <span className="font-medium text-right">
                    {pinReveal.cashier.data.firstName}{" "}
                    {pinReveal.cashier.data.lastName}
                  </span>
                </div>
                <div className="flex justify-between items-start gap-3 text-sm">
                  <span className="text-muted-foreground">Email</span>
                  <span className="font-medium text-right truncate">
                    {pinReveal.cashier.data.email || "—"}
                  </span>
                </div>
                <div className="flex justify-between items-start gap-3 text-sm">
                  <span className="text-muted-foreground">Outlets</span>
                  <span className="font-medium text-right">
                    {pinReveal.cashier.data.assignments
                      .map((a) => a.outletName)
                      .join(", ") || "—"}
                  </span>
                </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    size="icon"
                    className="h-12 w-12"
                    onClick={() => copyPin(pinReveal.cashier.pin)}
                    title="Copy PIN"
                  >
              ...
              <div className="flex items-start gap-2 rounded-md border border-warning/30 bg-warning/10 p-3 text-xs">
                ...
                <span>
                  For security, the PIN cannot be retrieved later. If lost,
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              and emailed to them. The current PIN will be invalidated
              immediately.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              will be unable to log into POS terminals and any active sessions
              will be terminated immediately. They'll see an access-denied
              message if they try to sign in.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() =>
                suspendTarget && suspendCashier({ status: "suspended" })
              }
